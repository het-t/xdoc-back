...
export class SyncRecordValuesController extends BaseController {
    constructor(
        ...
        private readonly loadCollectionById: ICollectionLoadById
    ) {
        super();
    }
    ...
    async execute(
        ...
        await Promise.allSettled(
            requests.map(async (request: {pointer: IPointer, version?: number}) => {
                ...
                switch(pointer.table) {
                    case "block": 
                        recordValue = await this.loadBlockById.execute(pointer.id);
                        break;

                    case "collection": 
                        recordValue = await this.loadCollectionById.execute(pointer.id);
                        break;                    
                }
                    ...
                    recordMap["block"][pointer.id] = {
                        ...
                        "value": {
                            "value": {
                                ...
                                "parent_id": "37b06f5d-e5ae-408f-b12a-ad1294ff1814",
                                "parent_table": "block",
                                "alive": true,
